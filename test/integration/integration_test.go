...
func TestIntegrationSuite(t *testing.T) {
	...
	t.Run("Tests", func(t *testing.T) {
		...
		t.Run("ReposerverServesBinaries", func(t *testing.T) {
			...
			versions := []string{"3.0.2", "3.3.4", "3.7.1"}
		...
		t.Run("ReposerverServesTools", func(t *testing.T) {
			// Check catalog tool
			resp, err := http.Get(tc.reposerverURL + "/dbt-tools/catalog/3.7.1/linux/amd64/catalog")
			...
			// Check description
			resp, err = http.Get(tc.reposerverURL + "/dbt-tools/catalog/3.7.1/description.txt")
...
func TestReposerverDirectAccess(t *testing.T) {
	...
	t.Run("BinaryContent", func(t *testing.T) {
		resp, err := http.Get(tc.reposerverURL + "/dbt/3.7.1/linux/amd64/dbt")
		require.NoError(t, err)
		defer resp.Body.Close()

		body, err := io.ReadAll(resp.Body)
		require.NoError(t, err)
		assert.Contains(t, string(body), "dbt version 3.7.1", "binary should contain version string")
	})

	t.Run("ChecksumContent", func(t *testing.T) {
		resp, err := http.Get(tc.reposerverURL + "/dbt/3.7.1/linux/amd64/dbt.sha256")
		require.NoError(t, err)
		defer resp.Body.Close()
	...
	t.Run("SignatureContent", func(t *testing.T) {
		resp, err := http.Get(tc.reposerverURL + "/dbt/3.7.1/linux/amd64/dbt.asc")
		require.NoError(t, err)
		defer resp.Body.Close()
